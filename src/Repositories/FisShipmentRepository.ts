...
import { INTERPORT_COMPANY, INTERPORT_BRANCH } from "../Constants";
...
export class FisShipmentRepository {
    getFisShipment(): Promise<FisShipment[]> {
    ...
    getFisShipmentByOceanbill(oceanBill: string): Promise<FisShipment[]> {
        return new Promise((resolve, rejects) => {
            fireBird.attach(options.opt, (err, db) => {
                if (err) {
                    throw err;
                ...
                db.query(query, [INTERPORT_COMPANY, INTERPORT_BRANCH], (err, result) => {
                    if (err) {
                        throw err;
                    }
                    db.detach();
    ...
    getFrtContainers(shipment: string): Promise<FrtContainer[]> {
        return new Promise((resolve, rejects) => {
            fireBird.attach(options.opt, (err, db) => {
                if (err) {
                    throw err;
                }
                db.query("SELECT C_NUMBER, F_L, TYPE1, SEAL, RECEIPT_DATE, DEPOT_AVAILABLE, DEPOT_STORAGE_FROM, DATEOUT, WEIGHT, M3, PACKS, PKG_DESC FROM FRT_CONTAINER WHERE SHIPMENT = ? AND COMPANY = ? AND BRANCH = ?", [shipment, INTERPORT_COMPANY, INTERPORT_BRANCH], (err, result) => {
                    if (err) {
                        throw err;
                    }
                    db.detach();
    ...
    getFisShipmentByContainerNumber(containerNumbers: string): Promise<FisShipment[]> {
        return new Promise((resolve, rejects) => {
            fireBird.attach(options.opt, (err, db) => {
                if (err) {
                    throw err;
    ...
    getFrtJobs(shipment: string): Promise<FisJob[]> {
        return new Promise((resolve, rejects) => {
            fireBird.attach(options.opt, (err, db) => {
                if (err) {
                    throw err;
                }
                db.query("SELECT HOUSEBILL, SHIPMENT, JOB_UNIQUE, JOB, PORT_DEST, GOODS1, PKGS_MANIFEST, PKG_DESC, ACTUAL_KG, ACTUAL_M3, ACS_RELEASE FROM FIS_JOB WHERE SHIPMENT = ? AND COMPANY = ? AND BRANCH = ?", [shipment, INTERPORT_COMPANY, INTERPORT_BRANCH], (err, result) => {
                    if (err) {
                        throw err;
                    }
                    db.detach();
    ...
    getJobContainers(shipment: string, job: string): Promise<JobContainer[]> {
        return new Promise((resolve, rejects) => {
            fireBird.attach(options.opt, (err, db) => {
                if (err) {
                    throw err;
                }
                db.query("SELECT C_NUMBER, PACKS, KG, M3, DAMAGED_PACKS, PILLAGED_PACKS, PKGS_OUTTURN, PKGS_RELEASE, KG_OUTTURN, M3_OUTTURN FROM FIS_JOBCONT WHERE SHIPMENT = ? AND COMPANY = ? AND BRANCH = ? AND JOB = ?", [shipment, INTERPORT_COMPANY, INTERPORT_BRANCH, job], (err, result) => {
                    if (err) {
                        throw err;
                    }
                    db.detach();
